        ...
        <ul className="px-4 py-3 pb-8 flex flex-col divide-y divide-neutral-50">
          {members.map((m) => (
            <li key={m.id} className="flex items-center gap-3 py-3">
              <div className={cn('w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0', m.color)}>
                {m.initial}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm text-neutral-800">{m.name}</p>
              </div>
              <span className={cn(
